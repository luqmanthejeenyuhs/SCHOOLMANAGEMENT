...
<div class="card p-3 mb-3">
    <div class="btn-group btn-group-sm">
        @foreach(['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected', 'all' => 'All'] as $key => $label)
            <a href="{{ route('admin.leave_requests.index', ['status' => $key]) }}"
    ...
    <table class="table mb-0 align-middle">
        ...
        @forelse($requests as $r)
            <tr>
                ...
                <td>
                    <span class="badge bg-{{ match($r->status) { 'approved' => 'success', 'rejected' => 'danger', default => 'warning text-dark' } }}">
                        {{ ucfirst($r->status) }}
                    </span>
                    @if($r->reviewedBy)
            ...
            @endif
        @empty
            <tr><td colspan="7" class="text-center text-muted py-3">No {{ $status !== 'all' ? $status . ' ' : '' }}leave requests.</td></tr>
        @endforelse
        </tbody>
    </table>
</div>
