...
<head>
    <meta charset="UTF-8">
    <title>Report Cards — {{ $exam->schoolClass->name }} — {{ $exam->name }}</title>
    <style>
...
@foreach($rows as $row)
    @php $comment = $comments->get($row['student']->id); @endphp
    <div class="sheet">
        <div class="header">
        ...
        <table class="info">
            <tr>
                <td class="label">Name</td><td class="value">{{ $row['student']->user->name }}</td>
                <td class="label">Admission No.</td><td class="value">{{ $row['student']->admission_no }}</td>
            </tr>
            <tr>
                <td class="label">Class</td><td class="value">{{ $exam->schoolClass->name ?? '—' }}</td>
                <td class="label">Stream</td><td class="value">{{ $row['student']->section->name ?? '—' }}</td>
            </tr>
        </table>
